...
pub fn create_stdlib() -> env::Env {
    ...
    io_vars.insert("prln".into(), new_native_func(func::Arity::Multi, |_, args| {
        if args.is_empty() {
            println!();
        } else if args.len() == 1 {
            println!("{}", args[0]);
    ...
    io_vars.insert("pr".into(), new_native_func(func::Arity::Multi, |_, args| {
        if args.is_empty() {
        } else if args.len() == 1 {
            print!("{}", args[0]);
        } else {
    ...
    io_vars.insert("input".into(), new_native_func(func::Arity::None, |_, _| {
    ...
    sys_vars.insert("clock".into(), new_native_func(func::Arity::None, |_, _| {
        let ts = time::now().to_timespec();
        Ok((ts.sec * 1_000 + i64::from(ts.nsec) / 1_000_000).into())
    }));

    sys_vars.insert("panic".into(), new_native_func(func::Arity::Some(1), |_, args| {
        eprintln!("piccolo panic! {}", args[0]);
        std::process::exit(1);
    }));

    sys_vars.insert("assert".into(), new_native_func(func::Arity::Some(1), |_, args| {
        if !is_truthy(&args[0]) {
    ...
    sys_vars.insert("show_env".into(), new_native_func(func::Arity::None, |i, _| {
        println!("{}", i.env);
        Ok(Value::Nil)
    }));

    sys_vars.insert("exit".into(), new_native_func(func::Arity::None, |_, _| {
        std::process::exit(0);
    }));

    env.set("sys", Value::Instance(data::Instance::new(&module, sys_vars)));

    // arr

    let mut arr_vars = HashMap::new();

    arr_vars.insert("push".into(), new_native_func(func::Arity::Some(2), |_, args| {
        match args[0] {
            Value::Array(ref a) => {
                let mut na = a.clone();
            ...
            _ => Err(PiccoloError::new(ErrorKind::IndexError, "Cannot push onto a non-array", 0))
        }
    }));

    arr_vars.insert("pop".into(), new_native_func(func::Arity::Some(1), |_, args| {
        match args[0] {
            Value::Array(ref a) => {
                let mut na = a.clone();
            ...
            _ => Err(PiccoloError::new(ErrorKind::IndexError, "Cannot pop from a non-array", 0))
        }
    }));

    arr_vars.insert("insert".into(), new_native_func(func::Arity::Some(3), |_, args| {
        match args[0] {
            Value::Array(ref a) => match args[2] {
                ...
                _ => Err(PiccoloError::new(ErrorKind::IndexError, "Cannot index with non-integer", 0))
            },
            _ => Err(PiccoloError::new(ErrorKind::IndexError, "Cannot insert into a non-array", 0))
        }
    }));

    arr_vars.insert("set".into(), new_native_func(func::Arity::Some(3), |_, args| {
        match args[0] {
            Value::Array(ref a) => match args[2] {
                Value::Integer(n) => {
                    if (n as usize) < a.len() {
    ...
    env.set("arr", Value::Instance(data::Instance::new(&module, arr_vars)));
