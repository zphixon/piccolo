...
impl super::ExprVisitor for NewEmitter {
    ...
    fn visit_unary(&mut self, _op: &Token, rhs: &Expr) -> Self::Output { rhs.accept(self); }
    fn visit_binary(&mut self, lhs: &Expr, op: &Token, rhs: &Expr) -> Self::Output {
        ...
        match op.kind {
            ...
            TokenKind::NotEqual => { self.0.write(Opcode::Equal, 1); self.0.write(Opcode::Not, 1); },
    ...
    fn visit_paren(&mut self, value: &Expr) -> Self::Output { value.accept(self); }
    fn visit_variable(&mut self, _name: &Token) -> Self::Output { unimplemented!() }
    fn visit_assign(&mut self, _name: &Token, value: &Expr) -> Self::Output { value.accept(self); }
    fn visit_logical(&mut self, lhs: &Expr, _op: &Token, rhs: &Expr) -> Self::Output { lhs.accept(self); rhs.accept(self); }
    fn visit_call(&mut self, _callee: &Expr, _paren: &Token, _arity: super::Arity, _args: &[Expr]) -> Self::Output { unimplemented!() }
    fn visit_new(&mut self, _name: &Token, _args: &[(Token, Box<Expr>)]) -> Self::Output { unimplemented!() }
    fn visit_get(&mut self, _object: &Expr, _name: &Token) -> Self::Output { unimplemented!() }
    fn visit_set(&mut self, _object: &Expr, _name: &Token, _value: &Expr) -> Self::Output { unimplemented!() }
    fn visit_index(&mut self, _rb: &Token, _object: &Expr, _idx: &Expr) -> Self::Output { unimplemented!() }
    fn visit_func(&mut self, _name: &Token, _args: &[Token], _arity: super::Arity, _body: &[Stmt], _method: bool) -> Self::Output { unimplemented!() }
}
